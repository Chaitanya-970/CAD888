...
/** Generate a grounded explanation from the backend's explanationInput */
function buildExplanation(expl: { unlitCells: number; totalCells: number; reportsLast30d: number; worstCellScore: number; timeBucket: string }): string {
  ...
  if (litPct >= 80) {
    parts.push(`Well-lit route with ${litPct}% of segments having good lighting`)
  } else if (litPct >= 50) {
    parts.push(`Route has moderate lighting coverage (${litPct}%)`)
  } else {
    parts.push(`Caution: only ${litPct}% of this route is well-lit`)
  }

  if (expl.reportsLast30d === 0) {
    parts.push('no safety concerns reported in the last 30 days')
  } else if (expl.reportsLast30d <= 2) {
    parts.push(`${expl.reportsLast30d} report${expl.reportsLast30d > 1 ? 's' : ''} filed nearby recently`)
  } else {
    parts.push(`${expl.reportsLast30d} reports filed in the last 30 days — exercise caution`)
  }

  if (expl.timeBucket === 'night') {
    parts.push('scores adjusted for nighttime conditions')
  }
